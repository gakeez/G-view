...
Component({
    ...
    attached: function () {
      ...
      query.select('#clock') // 在当前页面下选择第一个匹配选择器 selector 的节点。返回一个 NodesRef 对象实例，可以用于获取节点信息。
        ...
        .exec((res) => { // 执行所有的请求。请求结果按请求次序构成数组，在callback的第一个参数中返回。
          ...
          // 每隔一秒生成一次指针
          setInterval(this.draw, 1000, ctx, r, rem, res[0].width, res[0].height, this);
        })
    },
    detached: function () {
      // 在组件实例被从页面节点树移除时执行
  ...
  methods: {
    /* 
     生成指针
    */
   draw(ctx, r, rem, width, height, that) {
    // 每次生成都要清楚上次留下的内容
    ...
    that.drawBackground(ctx, r, rem); // 生成背景
    // 生成指针
    that.drawHour(ctx, r, hour, minute, rem);
    that.drawMinute(ctx, r, minute, rem);
    that.drawSecond(ctx, r, second, rem);
    that.drawDot(ctx); // 生成圆点
    ctx.restore();
  },
